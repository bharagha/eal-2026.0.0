...
DLSTREAMER_URL = (
    "https://github.com/open-edge-platform/edge-ai-libraries/tree/main/libraries/dl-streamer/"
)
...
_args.add_argument(
    "-omz",
    "--open_model_zoo",
    help="Required. Path to Open Model Zoo cloned repo",
    required=True,
    type=str,
)
_args.add_argument(
    "-mi", "--model_index", help="Required. Path to model_index.yaml file", required=True, type=str
)
_args.add_argument(
...
_args.add_argument(
    "-a",
    "--all",
    help="If true, table will contain all models, not only supported",
    required=False,
    type=str,
)
...
for root, dirs, files in os.walk(args.open_model_zoo + "/models", topdown=False):
    ...
    if name in models:
        ...
        models[name]["readme"] = (
            "https://docs.openvino.ai/latest/omz_models_model_"
    ...
    with open(os.path.join(accuracy_checker, file)) as f:
        ...
        for m in models_list:
            ...
            if name in models:
                ...
                print(
                    "WARNING: model listed in OV accuracy_checker but not listed in OV model zoo:",
                    name,
                )
        ...
        for name, m in model_index.items():
            ...
            if name in models:
                ...
                if "source" not in m and name + "-decoder" not in models:
                    print(
                        "WARNING: model listed in model_index.yaml but not listed in OV model zoo:",
                        name,
                    )
                if "format" not in m:
...
with open(args.output, "w") as f:
    ...
    for m in models:
        ...
        dlstreamer_support = m.get("dlstreamer_support", "")
        openvino_devices = m.get("openvino_devices", "?")
        pytorch_devices = m.get("pytorch_devices", "")
        tf_devices = m.get("tf_devices", "")
        f.write(f"    * - {n}\n")
        f.write(f"      - {task_type}\n")
