...
libgst.gst_structure_set_array.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_void_p]
...
libgst.gst_structure_get_array.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.POINTER(G_VALUE_ARRAY_POINTER),
]
...
def is_vaapi_buffer(_buffer):
    ...
    if mem is None:
        return False
    res = libgst.gst_memory_is_type(mem, GST_VAAPI_VIDEO_MEMORY_NAME.encode("utf-8"))
    # gst_memory_unref
...
libgstvideo.gst_video_region_of_interest_meta_get_param.argtypes = [
    VIDEO_REGION_OF_INTEREST_POINTER,
    ctypes.c_char_p,
]
