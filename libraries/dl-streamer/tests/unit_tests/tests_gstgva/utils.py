...
def get_model_proc_path(model_name):
    ...
    if models_procs_path:
        ...
        gst_va_path = pwd[: pwd.find("gst-video-analytics") + len("gst-video-analytics")]
...
class BBox:
    def __init__(
        self, x_min, y_min, x_max, y_max, additional_info=None, class_id=0, tracker_id=None
    ):
    ...
    def IoU(bbox_1, bbox_2):
        if (
            bbox_1.x_max < bbox_2.x_min
            or bbox_2.x_max < bbox_1.x_min
            or bbox_1.y_max < bbox_2.y_min
            or bbox_2.y_max < bbox_1.y_min
        ):
            return 0
        ...
        bbox_1_area = (bbox_1.x_max - bbox_1.x_min) * (bbox_1.y_max - bbox_1.y_min)
        bbox_2_area = (bbox_2.x_max - bbox_2.x_min) * (bbox_2.y_max - bbox_2.y_min)
    ...
    def additional_info_is_equal(pr_info, gt_info):
        ...
        for pr_info_item, gt_info_item in zip(pr_info, gt_info):
            ...
            if "name" in pr_info_item and "name" in gt_info_item:
                ...
                if (
                    pr_name == gt_info_item["name"]
                    and pr_name in supported_names
    ...
    @staticmethod
    def bboxes_is_equal(
        pr_bboxes: list, gt_bboxes: list, only_number=False, check_additional_info=True
    ):
