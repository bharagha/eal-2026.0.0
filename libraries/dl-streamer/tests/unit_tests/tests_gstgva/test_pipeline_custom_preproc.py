...
C_MODEL_PROC_PATH = os.path.join(SCRIPT_DIR, "test_files", "imagenet_custom_pre_proc_resnet.json")
...
C_GOLD_TRUE = [BBox(0, 0, 1, 1, [{"label": "espresso", "layer_name": "prob", "name": "ANY"}])]
...
class TestCustomPreProcPipeline(unittest.TestCase):
    def test_custom_opencv_yolo_11_pipeline(self):
        ...
        pipeline_runner.set_pipeline(D_OPENCV_PIPELINE_STR, IMAGE_PATH, D_GOLD_TRUE)
    ...
    def test_custom_vaapi_yolo_11_pipeline(self):
        pipeline_runner = TestPipelineRunner()
        pipeline_runner.set_pipeline(D_VAAPI_PIPELINE_STR, IMAGE_PATH, D_GOLD_TRUE)
        pipeline_runner.run_pipeline()
        for e in pipeline_runner.exceptions:
            print(e)
        pipeline_runner.assertEqual(
    ...
    def test_custom_vaapi_surface_sharing_yolo_11_pipeline(self):
        pipeline_runner = TestPipelineRunner()
        pipeline_runner.set_pipeline(D_VAAPI_SURFACE_SHARING_PIPELINE_STR, IMAGE_PATH, D_GOLD_TRUE)
        pipeline_runner.run_pipeline()
        for e in pipeline_runner.exceptions:
            print(e)
        pipeline_runner.assertEqual(
    ...
    def test_custom_opencv_resnet_pipeline(self):
        pipeline_runner = TestPipelineRunner()
        pipeline_runner.set_pipeline(C_OPENCV_PIPELINE_STR, IMAGE_PATH, C_GOLD_TRUE)
        pipeline_runner.run_pipeline()
        for e in pipeline_runner.exceptions:
            print(e)
        pipeline_runner.assertEqual(
    ...
    def test_custom_vaapi_resnet_pipeline(self):
        pipeline_runner = TestPipelineRunner()
        pipeline_runner.set_pipeline(C_VAAPI_PIPELINE_STR, IMAGE_PATH, C_GOLD_TRUE)
        pipeline_runner.run_pipeline()
        for e in pipeline_runner.exceptions:
            print(e)
        pipeline_runner.assertEqual(
    ...
    def test_custom_vaapi_surface_sharing_resnet_pipeline(self):
        pipeline_runner = TestPipelineRunner()
        pipeline_runner.set_pipeline(C_VAAPI_SURFACE_SHARING_PIPELINE_STR, IMAGE_PATH, C_GOLD_TRUE)
        pipeline_runner.run_pipeline()
        for e in pipeline_runner.exceptions:
            print(e)
        pipeline_runner.assertEqual(
