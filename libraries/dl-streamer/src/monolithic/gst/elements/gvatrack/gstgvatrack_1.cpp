...
static GstFlowReturn gst_gva_track_transform_ip(GstBaseTransform *trans, GstBuffer *buf) {
    ...
    if (gva_track && gva_track->tracker) {
        try {
            ...
            // Debug: Print frame format information
            if (0 && gva_track->info) {
                const gchar *format_name = gst_video_format_to_string(GST_VIDEO_INFO_FORMAT(gva_track->info));
                g_print("Processing frame: %dx%d, format=%s (%d), fps=%d/%d", GST_VIDEO_INFO_WIDTH(gva_track->info),
                        GST_VIDEO_INFO_HEIGHT(gva_track->info), format_name ? format_name : "unknown",
                        GST_VIDEO_INFO_FORMAT(gva_track->info), GST_VIDEO_INFO_FPS_N(gva_track->info),
                        GST_VIDEO_INFO_FPS_D(gva_track->info));

                // Debug: Print dlstreamer format
                g_print("DLStreamer format: %d, num_tensors: %d", static_cast<int>(gstbuffer->format()),
                        static_cast<int>(gstbuffer->num_tensors()));
            }
