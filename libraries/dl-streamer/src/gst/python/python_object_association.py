...
class Identifier(GstBase.BaseTransform):
    ...
    __gsttemplates__ = (
        Gst.PadTemplate.new("sink", Gst.PadDirection.SINK, Gst.PadPresence.ALWAYS, VIDEO_CAPS),
...
\t\tthe oldest samples when the budget is reached.""",
    ...
    def __init_on_start(self):
        ...
        metric = NearestNeighborDistanceMetric("cosine", self._max_iou_distance, self._nn_budget)
        ...
        self.__write_result = (
            self.__rewrite_regions_with_tracks if self._rewrite_roi else self.__write_ids_to_regions
        )
    ...
    def __get_detections(self, regions):
        ...
        for region in regions:
            ...
            detections.append(Detection(bounding_box, confidence, embedding))
    ...
    def __rewrite_regions_with_tracks(self, dst_vf, regions, tracks):
        ...
        with warnings.catch_warnings():
            ...
            for track in tracks:
                region = dst_vf.add_region(*track.to_tlwh(), label=self._label_to_save)
    ...
    def do_transform_ip(self, in_buffer: Gst.Buffer):
        try:
            ...
            regions = [
                r
                for r in dst_vf.regions()
                if self._object_class and r.label() == self._object_class or not self._object_class
            ]
...
__gstelementfactory__ = ("python_object_association", Gst.Rank.NONE, Identifier)
