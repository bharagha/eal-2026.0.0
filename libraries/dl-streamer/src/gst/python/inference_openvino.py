...
class InferenceOpenVINO(GstBase.BaseTransform):
    ...
    def compile_model(self):
        if not self.compiled_model:
            self.compiled_model = self.core.compile_model(self.model, self.property["device"])
            if self.property["nireq"] != 1:
                self.infer_queue = AsyncInferQueue(self.compiled_model, self.property["nireq"])
    ...
    def do_generate_output(self):
        ...
        tensors = [
            np.ndarray(shape=info.shape, buffer=map.data, dtype=np.uint8)
    ...
    def push_results(self, src, mems, maps, tensors):
        ...
        for tensor in tensors:
            mem = Gst.Memory.new_wrapped(0, tensor.tobytes(), tensor.nbytes, 0, None, None)
    ...
    TYPE_NAME = {"u8": "uint8", "f32": "float32"}


GObject.type_register(InferenceOpenVINO)

__gstelementfactory__ = ("inference_openvino", Gst.Rank.NONE, InferenceOpenVINO)
